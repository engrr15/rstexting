...
class Auth_model extends CI_Model {
	 ...
	 */
	 function loginAsAdmin($conditions=array())
	 {
		...
		if($result->num_rows()>0)
         ...
         function passwordRecovery($conditions=array())
	 {
	 	if(count($conditions)>0)		
	 		$this->db->where($conditions);
			 
	 	$this->db->select('users.Us_id, users.Us_email');
		$result = $this->db->get('users');
		...
		if($result->num_rows()>0)
         ...
         function lockedAsAdmin($conditions=array())
	 {
	 	if(count($conditions)>0)		
	 		$this->db->where($conditions);
	 ...
	 */
	 function setAdminSession($conditions=array())
	 {
	 	...
	 	$this->db->select('users.Us_id,users.Us_email, users.Us_group');
		$result = $this->db->get('users');
		if($result->num_rows()>0)
		{
			$row = $result->row();
			$values = array ('admin_id'=>$row->Us_id,'admin_name'=>$row->Us_email,'Us_group'=>$row->Us_group,'logged_in'=>TRUE,'admin_role'=>'admin'); 
